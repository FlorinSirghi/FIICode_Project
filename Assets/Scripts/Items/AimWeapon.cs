...
public class AimWeapon : MonoBehaviour
{
	...
	private void Shoot()
	{
		if(Input.GetMouseButtonDown(0))
		{
			...
			if(player.transform.localScale.x == 1)
			{
				OnShoot?.Invoke(this, new OnShootEventArgs { hitZombie = battleManagerScript.getFirstEnemyRight()});
			}
			else
			{
				OnShoot?.Invoke(this, new OnShootEventArgs { hitZombie = battleManagerScript.getFirstEnemyLeft()});
			}
